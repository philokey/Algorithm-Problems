...
struct circle
{
    ...
    double areatriangle(point a,point b)
    {
        ...
        q[len++]=b;
        if (len==4&&(dcmp(q[0].sub(q[1]).dot(q[2].sub(q[1])))>0))swap(q[1],q[2]);
        double res=0;
        int i;
        for (i=0;i<len-1;i++)
        {
...
struct polygon
{
    ...
    struct cmp
    {
        ...
        bool operator()(const point &aa,const point &bb)
        {
            ...
            if (d==0)
            {
                return dcmp(a.distance(p)-b.distance(p))<0;
            }
            return d>0;
        }
    };
    ...
    int pointinpolygon(point q)//点在凸多边形内部的判定
    {
        ...
        //if (n<=10)return relationpoint(q)>
        ...
        {
            mid=(low+high)>>1;
            if (dcmp(q.sub(p[0]).det(p[mid].sub(p[0])))>=0&&dcmp(q.sub(p[0]).det(p[mid+1].sub(p[0])))<0)
            {
                polygon c;
                c.p.resize(3);
                c.p[0]=p[mid];
    ...
    polygon minkowskisum(polygon a)
    {
        int i,j,ia=0,ib=0;
        for (i=0;i<n;i++)
        {
            if (dcmp(p[i].y-p[ia].y)<0||(dcmp(p[i].y-p[ia].y)==0&&dcmp(p[i].x-p[ia].x)<0))ia=i;
        }
        for (i=0;i<a.n;i++)
        {
            if (dcmp(a.p[i].y-a.p[ib].y)<0||(dcmp(a.p[i].y-a.p[ib].y)==0&&dcmp(a.p[i].x-a.p[ib].x)<0))ib=i;
        }
        polygon b;
        b.n=0;
        b.add(p[ia].add(a.p[ib]));
        //puts("fi");
        //p[ia].output();
        //a.p[ib].output();
        //b.p[0].output();
        ...
        while (i<n||j<a.n)
        {
            ...
            else
            {
                int k=(ia+i)%n;
                int nk=(k+1)%n;
                double va=atan2(p[nk].y-p[k].y,p[nk].x-p[k].x);
                k=(ib+j)%a.n;
                nk=(k+1)%a.n;
                double vb=atan2(a.p[nk].y-a.p[k].y,a.p[nk].x-a.p[k].x);
                if (dcmp(va)<0)va+=2*pi;
                if (dcmp(vb)<0)vb+=2*pi;
                t=(va>vb);
            }
            if (t)
            {
                int k=(ib+j)%a.n;
                int nk=(k+1)%a.n;
                b.add(a.p[nk].sub(a.p[k]).add(b.p[b.n-1]));
                j++;
            }
            else
            {
                int k=(ia+i)%n;
                int nk=(k+1)%n;
                b.add(p[nk].sub(p[k]).add(b.p[b.n-1]));
                i++;
            }
        }
        b.n--;
...
struct polygons
{
    ...
    double polyareaunion()
    {
        ...
        for (i=0;i<p.size();i++)
        {
            if (p[i].getdir()==0)reverse(p[i].p.begin(),p[i].p.end());
        }
        for (i=0;i<p.size();i++)
        {
            for (k=0;k<p[i].n;k++)
...
struct halfplanes
{
    ...
    void unique()
    {
        ...
        for (i=1;i<n;i++)
        {
            ...
            else if (dcmp(hp[m-1].b.sub(hp[m-1].a).det(hp[i].a.sub(hp[m-1].a))>0))hp[m-1]=hp[i];
        }
        n=m;
    }
    bool halfplaneinsert()
    {
        ...
        while (st<ed&&dcmp(hp[que[st]].b.sub(hp[que[st]].a).det(p[ed].sub(hp[que[st]].a)))<0)ed--;
...
struct point3
{
    ...
    {
        return dcmp(a.x-x)==0?dcmp(y-a.y)==0?dcmp(z-a.z)<0:y<a.y:x<a.x;
    }
    double len()
    {
        return sqrt(len2());
...
struct line3
{
    ...
    {
        return dcmp(p.sub(a).det(p.sub(b)).len())==0&&dcmp(a.sub(p).dot(b.sub(p)))<=0;
    }
...
struct plane
{
    ...
    int pointontriangle(point3 p)//点是否在空间三角形abc上
    {
        ...
        if (dcmp(s-s1-s2-s3))return 0;
        if (dcmp(s1)&&dcmp(s2)&&dcmp(s3))return 2;
        return 1;
    }
    //判断两平面关系
    //0 相交
    ...
    {
        if (dcmp(o.det(f.o).len()))return 0;
...
int main()
{
    ...
    */
    ...
    while (cas--)
    {
        ...
        p.input();
        p=p.mul(3.0);
        puts(a.pointinpolygon(p)!=-1?"YES":"NO");
    }
    return 0;
}
